...
checklist_ids <- read_sampling(paste0(data_folder, "ebd_sampling_", data_tag, ".txt")) %>%
...
sampling_events <- read_csv(paste0(data_folder, "ebd_", data_tag, "_zf.csv"), na = "") %>%
...
prediction_data <- select(richness2, contains("pland")) %>%
                  ...
                  summarise_all(mean) %>%
                  mutate(hab_diversity = gs(pland_00:pland_16)) %>%
...
write_csv(results, "data/cci_", data_tag, ".csv")
